...
//app.use(allowCrossDomain);
  try {
    ...
    res.status(200).json({ message: `Authentification réussie`,token: `${token}`, user});
  } catch (error) {
...
exports.createUser = async (req, res) => {
  ...
  try {
    const newUser = await userService.createUser(nom, prenom, grade, unite, password);
    res.status(201).json(newUser);
  } catch (error) {
    console.error("Erreur lors de la création de l'utilisateur :", error);
    res.status(500).json({ message: "Impossible de créer un nouvel utilisateur." });
  }
};
...
exports.signup = async (req, res) => {
  ...
  try {
    const utilisateurExiste = await userService.verifierUtilisateur(nom, prenom, grade, unite);
    if (utilisateurExiste) {
      return res.status(400).json({ message: "Un utilisateur avec ces attributs existe déjà." });
    }
    const newUser = await userService.createUser(nom, prenom, grade, unite, password);
    res.status(201).json(newUser);
  } catch (error) {
...
exports.resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: 'Aucun utilisateur trouvé avec cette unité.' });
    }

    res.status(200).json({ message: 'Un nouveau mot de passe temporaire a été envoyé à votre adresse e-mail.' });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.verifierUtilisateur = async (req, res) => {
  ...
  try {
      const utilisateurExiste = await userService.verifierUtilisateur(nom, prenom, grade, unite);
      if (utilisateurExiste) {
          res.status(200).json({ message: "Utilisateur trouvé dans la base de données." });
      } else {
